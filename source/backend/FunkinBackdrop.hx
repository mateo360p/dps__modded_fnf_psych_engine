 ...
 */
class FunkinBackdrop extends FlxSprite
{
	/**
	...
	function regenGraphic(camera:FlxCamera)
	{
		...
		var bufferX = 0; 
		bufferX = (repeatAxes.x && angle != 0) ? 1 : 0;
		var bufferY = 0;
		bufferY = (repeatAxes.y && angle != 0) ? 1 : 0;
		for (tileX in Std.int(-bufferX)...tilesX + bufferX)
		{
			for (tileY in Std.int(-bufferY)...tilesY + bufferY)
			{
