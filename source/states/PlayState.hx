...
{
	//Double Ghost Stuff noteRows
	var noteRows:Array<Array<Array<Note>>> = [[],[]];
	public static var songAllowedDrain:Bool = true;
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Leave, please', 0.2], //From 0% to 19%
		['Can you do it better?', 0.4], //From 20% to 39%
		['Uhhh', 0.5], //From 40% to 49%
		['Thats bad', 0.6], //From 50% to 59%
		['Not too bad', 0.69], //From 60% to 68%
		['Not bad', 0.7], //69%
		['Finger enthusiast', 0.8], //From 70% to 79%
		['Finger pro', 0.9], //From 80% to 89%
		['Finger Master', 0.95], //From 90% to 94%
		['Finger GOD', 1], //From 95% to 99%
		['Roger Roger!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	 ...
	 * [ songName, player]
	 */
	public static var storyPlaylist:Array<Array<String>> = [];
	...
	private var curHealth:Float = 1; //for a smoother healthbar ;D
	...
	// Extra key
	public var heyKey:Array<FlxKey>;
	public var dodgeKey:Array<FlxKey>;
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, healthBar.y - 90, FlxG.width - 800, Language.getPhrase("botplay", "Hack activated").toUpperCase(), 32);
	...
	override public function update(elapsed:Float)
	{
		...
		curHealth = FlxMath.lerp(curHealth, health, 24 / ClientPrefs.data.framerate);
	...
	public function endSong()
	{
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			Highscore.saveScore(FreeplayState.player + "_" + Song.loadedSongName, songScore, storyDifficulty, percent);
			#end
			playbackRate = 1;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0) // End of the week
				{
					Mods.loadTopMod();
				...
				{
					// Yeah, fck organization
					var difficulty:String = Difficulty.getFilePath();
					var firstSong:Array<String> = [storyPlaylist[0][0].toLowerCase(), storyPlaylist[0][1].toLowerCase()];
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		var drainHealth:Bool = true; // prevent health gain, *if* sustains are treated as a singular note
		if (guitarHeroSustains && note.isSustainNote) drainHealth = false;
		if (drainHealth && health > 0.05 && songAllowedDrain) health -= note.hitHealth * healthGain;
	}
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (ClientPrefs.data.sectionBeat && camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.data.camZooms)
			{
