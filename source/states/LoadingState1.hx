...
class LoadingState1 extends MusicBeatState
{
	...
	public static function checkLoaded():Bool
	{
		...
		//trace(initialThreadCompleted); //THIS IS THE REAL SHIT
	...
	static function getNextState(target:FlxState, stopMusic = false, intrusive:Bool = true):FlxState
	{
		loadNextDirectory();
		if(intrusive)
			return new LoadingState1(target, stopMusic);

		if (stopMusic && FlxG.sound.music != null)
			FlxG.sound.music.stop();
		
		trace("I TRIED SOOO HARDDDD AND GET SOOO FARRRRRRRRRRRRRRR");
		while(true)
		{
			if(!checkLoaded())
			{
		...
		trace("BUT IN THE ENDDDDDDDDDDDDDDDDD");
		return target;
	}
	...
	public static function prepareToSong()
	{
		...
		new Future<Bool>(() -> {
			...
			catch(e:Dynamic) { trace("Log: Song json preload file doesn't exist");}
			return true;
		}, true)
		.then((_) -> new Future<Bool>(() -> {
			...
			trace(songsToPrepare);
			trace("songs are good, iguess...");
			if (player2 != player1)
			{
				threadsMax++;
				threadPool.run(() -> {
					try { preloadCharacter(player2); } catch (e:Dynamic) { trace("OPP ERROR: " + e);}
					completedThread();
				});
			}
			trace("oppt completed");
			if (!stageData.hide_girlfriend && gfVersion != player2 && gfVersion != player1)
			{
				threadsMax++;
				threadPool.run(() -> {
					try { preloadCharacter(gfVersion); } catch (e:Dynamic) {trace( "GF ERROR:" + e);}
					completedThread();
				});
			}
			trace("gf completed");
	...
	public static function startThreads()
	{
		...
		trace(imagesToPrepare.toString() + soundsToPrepare.toString() + musicToPrepare.toString() + songsToPrepare.toString());
		loaded = 0;
