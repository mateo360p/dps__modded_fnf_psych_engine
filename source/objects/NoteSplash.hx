...
class NoteSplash extends FlxSprite
{
	...
	{	
		...
		if (config.allowRGB)
		{
			...
			Note.initializeNoteTypeRGBShader(note.noteType, noteData % Note.colArray.length);
			function useDefault()
			{
				tempShader = Note.globalRgbShaders[noteData % Note.colArray.length];
			}

			function useFromNotetype()
			{
				tempShader = Note.noteTypeRgbShaders[noteData % Note.colArray.length];
			}

			if(((cast FlxG.state) is NoteSplashEditorState) || ((note.noteSplashData.useRGBShader) && (PlayState.SONG == null || !PlayState.SONG.disableNoteRGB)))	// If Note RGB is enabled:
			{
				if (note.noteSplashData.useNoteTypeShader){
					useFromNotetype();
				}
				else if((!note.noteSplashData.useGlobalShader || ((cast FlxG.state) is NoteSplashEditorState))) //you shouldn't use this, in my opinion
				{
						...
						for (i in 0...colors.length)
						{
							...
							if (rgb == null)
							{
								if (i == 0) {
									...
									if (ClientPrefs.data.noteSkin.toLowerCase() == 'dark'){
										tempShader.g = 0xFF000000;
									} else {
										tempShader.g = arr[1];
									}
								} else if (i == 2) {
				...
				else if (note.noteSplashData.useNoteTypeShader) useFromNotetype();
				else useDefault();
			}
		}
		rgbShader.copyValues(tempShader, note.noteSplashData.useNoteTypeShader);
...
class PixelSplashShaderRef 
{
	...
	public function copyValues(tempShader:RGBPalette, usesNoteTypeRGB:Bool)
	{
		if(tempShader != null)
		{
			for (i in 0...3)
			{
				...
				if (ClientPrefs.data.noteSkin.toLowerCase() == 'dark' && !usesNoteTypeRGB){
					shader.g.value[i] = 0xFF000000;
				};
